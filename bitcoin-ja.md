...
Proof-of-workは、それをハッシュ化したとき、その値がいくつかの0ビットで始まっている必要がある。
それには要求される0ビットの数のべき乗の作業量が必要であるが、認証に必要なのは１回のハッシュ化のみである。
...
Proof-of-workはまた、多数派の代表を決定するという問題をも解決する。
仮にIPアドレスにつき１つの投票権を持つような仕組みによって多数派というのが規定されている場合、たくさんのIPを利用することができる人なら誰でも多数派を覆すことができる。
Proof-of-workとは本質的に、CPUにつき1つの投票権を持つようなものである。
多数派の選択は最長のチェーンという形で表現され、それは同時に最大の仕事による投票がなされたものでもある。
CPUパワーの過半数が誠実な参加者によってコントロールされている限り、正当なチェーンは最も早く成長し、競合するチェーンを凌ぐ。
過去のブロックを変更するためには、攻撃者はそのブロックとそれ以降の全てのブロックについて最新のブロックに追いつくまでproof-of-workをやり直す必要があるだろう。
...
もし異なる２つのノードがそれぞれに異なるブロックを同時にブロードキャストした場合、あるノードは一方を先に受け取り、あるノードはもう一方を先に受け取るかもしれない。
その場合、各ノードは最初に受け取ったブロックを正当なブロックとして扱い、そのブロックのチェーンを伸ばすように働く。
ただし、もう一方のブランチがより長くなった場合に備えて他方のブロックも保存しておく。
分岐点は次のproof-of-workが発見され、一方が他方より長くなった時点でなくなるだろう。
最長でないブランチに貢献していたノードは直ちにより長いブランチに移動する。

新規トランザクションのブロードキャストは、全てのノードに届く必要はない。
多数のノードに届いてさえすれば、そう長くないうちにそれらがブロックに取り込んでくれるだろう。
ブロックのブロードキャストはまた、その欠落にも耐性がある。
